...
public class KakaoMessageResendService implements MessageSendService {
    ...
    public boolean isSupported(MessageHandlerKey key) {
        return key.equals(MessageHandlerKey.KAKAO_MESSAGE_RETRY);
    }
        ...
        webClient.post()
                ...
                .doOnError(res -> {
                    ...
                    messageFailJpaRepository.save(
                            MessageFailEntity.create(MessageType.KAKAO, requestBody.toString())
                    );
                })
                .subscribe();
    }

    private void publishSmsEvent(String message) {
        messageEventProduceService.produceEvent(smsTopic, message);
    }
}
